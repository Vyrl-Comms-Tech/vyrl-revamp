...
export default function AboutPartnersSection({
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      ...
      requestAnimationFrame(() => {
        ...
        ctx.add(() => {
          ...
          //
          // The empty timeline is created synchronously here (inside
          // ctx.add()'s callback) purely so gsap.context can track it for
          // cleanup — the poll below only ever appends a .fromTo() onto
          // THIS already-tracked timeline rather than creating a brand
          // new standalone tween from inside a later requestAnimationFrame
          // callback, which would fall outside ctx's synchronous capture
          // and never get reverted on unmount. Same reason
          // Testimonials.jsx's own navbar poll appends onto its
          // already-tracked `introTl` instead of creating a fresh tween.
          ...
          const navbarTl = gsap.timeline({
            scrollTrigger: {
              ...
              // bottom edge touches the very bottom of the viewport,
              // i.e. as early as the section's bottom is even barely
              // visible. That read as too early/no delay. GSAP's second
              // value is how far down the VIEWPORT the trigger point
              // sits — "bottom bottom" = 100% down (the very edge);
              // moving that number DOWN to "90%" makes the element's
              // bottom edge need to travel further up past the viewport
              // before it counts, i.e. MORE scroll, which is what reads
              // as a delay here. Small, deliberate ~10% of a viewport's
              // worth of extra scroll before this starts.
              start: "bottom 90%",
              end: "bottom top",
              scrub: 1,
            },
    ...
    return () => {
      cancelled = true; // guards the deferred rAFs above if unmount beats them
      ctx.revert(); // reverts document.body's backgroundColor inline style
      // back to whatever it was before too — gsap.context tracks a tween
      // by creation, not by which element it targets, so it's covered
      // even though document.body lives outside sectionRef's subtree.
      firstShuffle.kill();
      startLoop.kill();
      loopCall?.kill();
      gsap.killTweensOf(imgRefs.current);
      gsap.set(document.body, { clearProps: "backgroundColor" }); // don't leak black bg to other routes
    };
  }, []);
