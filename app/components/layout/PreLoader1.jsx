...
//  *
//  * Two modes:
//  *  - Timed hold: stays opaque for `holdDuration` seconds, then fades out
//  *    (legacy behaviour, used when no `progress` is supplied).
//  *  - Progress-driven: pass `progress` (0-100) to show a live "Parsing
//  *    model... NN%" readout tied to real asset loading. The preloader only
//  *    fades out once `progress` reaches 100 (and `holdDuration` has elapsed,
//  *    if provided, as a minimum display time).
//  */
// const Preloader = ({
//     holdDuration = 2,
//     progress,
...
// }) => {
//     const containerRef = useRef(null)
//     const isProgressDriven = typeof progress === 'number'
//     const clampedProgress = isProgressDriven ? Math.min(100, Math.max(0, Math.round(progress))) : 0

//     useEffect(() => {
//         if (isProgressDriven) return // handled by the effect below
//         let cancelled = false
//         let fadeTween

//         const fadeOut = () => {
//             if (cancelled || !containerRef.current) return
//             fadeTween = gsap.to(containerRef.current, {
//                 opacity: 0,
//                 duration: 0.6,
//                 ease: 'power2.out',
...
//             const readyPromise = onReveal?.()
//             if (readyPromise?.then) {
//                 readyPromise.then(fadeOut)
//             } else {
//                 fadeOut()
//             }
//         }

//         const exitTl = gsap.timeline({ delay: holdDuration }).call(revealDestination)
...
//     // Progress-driven mode: wait for 100% (plus the minimum hold), then fade.
//     const firedRef = useRef(false)
//     useEffect(() => {
//         if (!isProgressDriven || clampedProgress < 100 || firedRef.current) return
...
//         const t = gsap.delayedCall(Math.max(0, holdDuration), revealDestination)
...
//             fadeTween?.kill()
//         }
//     }, [isProgressDriven, clampedProgress, holdDuration])
...
//                         ))}
//                     </h1>
//                     {subtitle && <p className="preloader-subtitle">{subtitle}</p>}
//                     <div className="preloader-bar">
//                         <div className="preloader-bar__fill" style={{ width: `${clampedProgress}%` }} />
//                     </div>
//                     <div className="preloader-status">
//                         <span className="preloader-status__label">{label}</span>
//                         <span className="preloader-status__percent">{clampedProgress}%</span>
//                     </div>
//                 </div>
//             ) : (
...
export default function Preloader({ onComplete, progress = 100 }) {
  const containerRef = useRef(null);
  const pathRef = useRef(null);
  ...
  return (
    <div
      ref={containerRef}
      className={`pre-loader2 ${isExiting ? "pre-loader2--exiting" : ""}`}
      role="status"
      aria-live="polite"
      aria-label="Page loading"
    >
